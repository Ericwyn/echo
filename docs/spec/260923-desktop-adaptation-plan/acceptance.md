...
当前应用代码 `67f62407` 已在全新 `build/linux-lldtmp-2054` CMake build-dir 构建，使用系统 Clang 与从 Ubuntu 包解压至 `/tmp` 的真实 LLD 14。当前 `.deb` 为 `1.1.0+2052`/amd64，SHA-256：`29715b8ae81432287418e431af6db55eabb842db8f3048053759fb8ea3bdd05b`；bundle 完整性检查和包元数据通过。Android arm64-only APK version name/code 为 `1.1.0`/`2043`，高于设备报告的 `2026`；release 证书校验通过，SHA-256：`007e78cbd0139aaebfd8e034c7aa981859500a1a646a4223f8b7fc25a2dc3d93`。产物未安装或启动，Flutter tests/analyze 未运行；手动交互由用户验收。详见 [添加音乐库播放器交接构建证据](evidence/260924-add-library-player-handoff-build/README.md)。
...
| A13 | Android 不退化（R1） | 手机导航/MiniPlayer/歌词/队列和通知栏、锁屏后台、转码 seek、恢复流程通过 | P1–P5 | arm64-only release APK 已用本机签名文件构建；version name/code 为 `1.1.0` / `2043`，高于用户设备报告的 `2026`；仅包含 `arm64-v8a`，签名和 package metadata 校验通过。尚未安装/启动，通知栏、锁屏、共享播放器与数据恢复仍待用户真机回归。 |
| A14 | 可安装可运行（R4） | 干净环境由安装包满足 native 依赖，应用菜单启动并实际播放；升级保留用户状态 | P5 | 部分实现（当前 `1.1.0+2052` bundle 与 `.deb` 已从全新 `build/linux-lldtmp-2054` 构建；打包前逐项检查 app/engine、项目 Linux 插件、native assets、ICU、Flutter manifests、许可证和托盘图标；ELF、amd64、desktop entry、依赖元数据通过；干净安装/实际播放/升级保留状态未测） |
...
| A20 | 音乐库切换隔离（P1/P4） | 切换音乐库时分别保存/恢复各自队列；旧音频停止、旧系统媒体项清除；AudioService 不重复初始化，失败后可重试 | P1/P4 | `6f0f80e6` 实现库作用域会话、切库停播/清理媒体元数据及 Android AudioService 单例重绑；`67f62407` 将新库认证纳入同一交接流程并在失败时回滚；相关回归用例未运行，切库成功/失败恢复与 Android 后台媒体控件待用户实测 |
