...
| A14 | 可安装可运行（R4） | 干净环境由安装包满足 native 依赖，应用菜单启动并实际播放；升级保留用户状态 | P5 | 部分实现（`cab6fe11` 最新 Linux bundle 与 `.deb` 在全新 `build/linux-lldtmp-2061` 构建；DEB 元数据为 `echoes`/`1.1.0+2053`/`amd64`；本机 ARM64 APK 为 `1.1.0`/versionCode `2051`，签名/ABI/包信息通过。未安装或启动，干净环境依赖、实际播放和升级状态仍待用户验证） |
