...
| A09 | 托盘与关闭（R4） | 关闭继续播放且可恢复；无宿主/宿主消失可找回；显式退出停止声音并清理实例 | P0-A/P4-B/C | 部分实现（`19c9fb4e` 加入关闭行为选择与活跃本地下载提示；`5df8140e` 让退出清理逐项限时并在失败时继续；`c2496e94` 在停止前保存恢复快照并避免 dispose 覆盖进度；`21cf973f` 修正 watcher 别名/初始查询竞态并在宿主重启后刷新 tray；`8f51f1fc` 首次关闭到后台前说明恢复方式，`94b7a2ab` 让托盘控制单次派发且跟随快照；`d6bb46f5`、`8acd46dd`、`bb8d9af3` 补上关闭/显示恢复策略；`0e832ddb` 增加 watcher 断线退避重连和窗口 destroy 失败恢复，Linux release 已编译；新增退避用例未运行，Ubuntu 手动场景仍待验证） |
