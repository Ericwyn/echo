...
| P5 | 当前 Linux `1.1.0+2051` bundle/.deb 与 Android arm64 APK 均已构建 | Linux 使用系统 Clang 与 Ubuntu LLD 14，在全新 `build/linux-lldtmp-2053` 构建；Debian 包版本 `1.1.0+2051`、amd64，bundle 完整性与包元数据已核对。Android 本机签名 ARM64 APK versionCode `2042`，高于设备报告的 `2026`；签名和包信息已校验。产物未安装或启动；用户仍需验收 A18 返回路径、Ubuntu 桌面行为、Android 后台播放与性能；Windows CI 暂缓。最新证据：[音乐库返回导航构建](evidence/260924-library-flow-navigation-build/README.md) |
