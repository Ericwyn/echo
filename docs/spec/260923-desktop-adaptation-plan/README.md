...
目前桌面侧栏、常驻播放条、歌词/队列工作区、Linux MPRIS 和托盘已有首轮实现。用户确认 Ubuntu 系统媒体卡片能控制播放并显示封面，托盘基础功能正常，音量条外观可接受。导航/窗口改造检查点为 `7f4ae955`；用户随后报告 Linux 自绘窗口栏点击异常和闪烁，代码已移除依赖路由 Overlay 的 Tooltip，并在启动时恢复普通窗口状态、最小化/最大化/缩放能力。Linux GTK runner 现已改为单实例并在再次启动时激活现有窗口，尚待用户实测。桌面与手机主导航、桌面侧栏及抽屉管理项现由 `AppNavigationModel` 共用稳定定义。检查点 `c5d2b512` 的最新 Linux release bundle 与 `.deb` 已重新构建，没有安装或启动；新增的导航、5000 首队列定位、MPRIS seek/error 和 AudioService 音量回退用例均未运行。最近桌面改动还移除账户底栏重复弹窗，将线路和音乐库新增入口归到设置页；返回历史、窗口栏交互、单实例、状态恢复、长队列实机性能、完整 MPRIS seek、托盘宿主异常、干净安装和 Android 最终实机回归仍待验收。按用户要求，不启动应用或运行测试；手动交互由用户执行。
...
| P3 | 首轮实现中 | 宽屏/紧凑工作区和桌面队列交互已实现；远端当前项定位改为按已渲染行高逐步校正，5000 首用例已加未运行；长队列性能采样和真实窗口验收待补 |
| P4 | Linux 基础可用 | 用户确认 MPRIS 控制/封面与托盘基础功能；单实例、关窗选择、下载退出提示及 SetPosition/Seeked/error 隔离已编译；MPRIS 用例已加但未跑，托盘宿主失效与恢复仍待用户验收 |
| P5 | `c5d2b512` 的 Linux bundle 与 `.deb` 已构建 | 尚未安装 `.deb` 或在干净 Ubuntu 播放；之后做 Android 回归；Windows CI 和 Windows 实机验收暂缓 |
