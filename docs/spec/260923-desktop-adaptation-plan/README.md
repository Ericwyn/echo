...
目前桌面侧栏、常驻播放条、歌词/队列工作区、Linux MPRIS 和托盘已有首轮实现。用户确认 Ubuntu 系统媒体卡片能控制播放并显示封面，托盘基础功能正常，音量条外观可接受。导航/窗口改造检查点为 `7f4ae955`；用户随后报告 Linux 自绘窗口栏点击异常和闪烁，代码已移除依赖路由 Overlay 的 Tooltip，并在启动时恢复普通窗口状态、最小化/最大化/缩放能力。Linux GTK runner 现已改为单实例并在再次启动时激活现有窗口，但尚未实测。用户授权的中间 Linux release bundle 在 `eb2edfa9` 成功构建，大小约 59 MB；随后新增的窗口尺寸/最大化状态恢复尚未包含在该 bundle 中。按用户要求，不启动应用、不运行测试；窗口栏交互、单实例、状态恢复和 Android 最终实机回归留给用户验收。长队列、完整 MPRIS seek、托盘宿主异常和干净安装也仍待验收。
...
| P2 | 桌面统一返回/前进栈和 Linux 自绘窗口栏已实现，等待用户复测 | 手机保留分支路由，桌面使用单 Navigator；桌面去掉重复搜索/返回/标题，四处 chrome 分隔线改用浅色 divider；窗口操作和视觉细节等用户验收 |
