...
| P4 | Linux 基础可用 | 用户确认 MPRIS 控制/封面与托盘基础功能；单实例、关窗选择、首次关闭到后台恢复提示、动态托盘控制、下载退出提示及 SetPosition/Seeked/error 隔离已编译；`0e832ddb` 增加 watcher 断线退避重连和 destroy 失败恢复；`e34563ef` 修复退出恢复与初始 watcher 查询的竞争；`9abb3e09` 抽出 show/minimize 兜底并新增 3 条回归定义，Linux release 已构建。新增用例未运行；关窗恢复、宿主重启和手动验收仍待用户完成 |
| P5 | Linux `1.1.0+2054` DEB 与 bundle ZIP、Android ARM64 APK 已构建 | Linux lifecycle recovery commit `9abb3e09` 后，发行版本提交 `5f3ed83b` 在全新 `build/linux-lldtmp-2065` 构建；DEB SHA-256 `74906068a616b764dc939f6594ed218e00309addb21c39cc57cee8bfe24adb9e`，standalone bundle ZIP SHA-256 `cafaa42f48442e8660ad692ef6a942ba99dfbc1cd0fa5311d202f2c36ccc0993`，归档完整性通过；DEB 声明 GTK、Ayatana AppIndicator、libmpv 运行依赖，当前主机直接 ELF 依赖检查无缺失；`dpkg` 确认 2054 高于上一 Linux 包 2053。Android 本机签名 ARM64 APK versionCode `2054`，高于用户设备版本 `2026`，仅含 `arm64-v8a`，签名及包信息校验通过。产物未安装或启动；手动与性能验收仍由用户执行；Windows CI 暂缓。最新 Linux 证据：[可升级 Linux 构建](evidence/260924-linux-upgradeable-release/README.md) |
