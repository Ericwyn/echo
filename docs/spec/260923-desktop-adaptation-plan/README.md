...
最近应用代码检查点：`c4bd0b6e`（共享 queue 命令限制越界起始索引）；前序 `46453bb2`、`b12aa608` 将队列播放、试听和歌曲动作接入 PlaybackCommands。Linux `1.1.0+2053` `.deb` 与 standalone bundle ZIP；Android 本机 ARM64 脚本产出 versionCode `2054`。本机签名脚本由 `.git/info/exclude` 忽略，不纳入提交。最新产物与检查记录见 [播放队列索引边界构建](evidence/260924-playback-queue-index-build/README.md)。
...
| P5 | 当前 Linux `1.1.0+2053` bundle/.deb 与 Android arm64 APK 均已构建 | `c4bd0b6e` 播放队列索引保护在 `build/linux-lldtmp-2063` 构建；DEB SHA-256 `955e3c8f222be6582a4ae9a9993fb5b700136cc61704401c5b06e413ed4a8ffd`，standalone bundle ZIP SHA-256 `5de9a27a5d1be3e15e3540d2750cb9323acbfd52c3f365e9e16c91dfd88f2bbd`，完整性检查通过。Android 本机签名 ARM64 APK versionCode `2054`，仅含 `arm64-v8a`，证书校验通过，SHA-256 `26ce4001832b034a515392661eafa0ddda4c80b08b2774c9c85f0b475f4d1380`。产物未安装或启动；手动与性能验收仍由用户执行；Windows CI 暂缓。最新证据：[播放队列索引边界构建](evidence/260924-playback-queue-index-build/README.md) |
