...
最近应用代码检查点：`e34563ef`（Linux 状态栏 watcher 退避重连及窗口退出失败恢复），前序 `c4bd0b6e` 补上共享队列起始索引保护。Linux 最新 `.deb` 为 `1.1.0+2053`/amd64，SHA-256 `aef145de81ef8dfe9de179150a6555ecde738632c2e67636015047f929c43701`；Android 本机签名 ARM64 APK versionCode `2054`，本机脚本由 `.git/info/exclude` 忽略。最新 Linux 证据见[生命周期恢复构建](evidence/260924-linux-lifecycle-recovery/README.md)。
...
| P4 | Linux 基础可用 | 用户确认 MPRIS 控制/封面与托盘基础功能；单实例、关窗选择、首次关闭到后台恢复提示、动态托盘控制、下载退出提示及 SetPosition/Seeked/error 隔离已编译；`0e832ddb` 增加 watcher 断线退避重连和 destroy 失败恢复；`e34563ef` 修复退出恢复与初始 watcher 查询的竞争，Linux release 已构建。新增用例未运行；关窗恢复、宿主重启和手动验收仍待用户完成 |
| P5 | Linux `1.1.0+2053` DEB 与 bundle ZIP、Android ARM64 APK 已构建 | Linux lifecycle recovery commit `e34563ef` 在 `build/linux-lldtmp-2064` 构建；DEB SHA-256 `aef145de81ef8dfe9de179150a6555ecde738632c2e67636015047f929c43701`，standalone bundle ZIP SHA-256 `6d9ad00606b37d66325c21c90d17c4b49b0f9bc48125b9dc103356fa7fa2e3c4`，归档完整性通过。Android 本机签名 ARM64 APK versionCode `2054`，仅含 `arm64-v8a`，证书校验通过。产物未安装或启动；手动与性能验收仍由用户执行；Windows CI 暂缓。最新 Linux 证据：[生命周期恢复构建](evidence/260924-linux-lifecycle-recovery/README.md) |
