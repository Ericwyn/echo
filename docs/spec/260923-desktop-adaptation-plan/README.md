...
目前桌面侧栏、常驻播放条、歌词/队列工作区、Linux MPRIS 和托盘已有首轮实现。用户确认 Ubuntu 系统媒体卡片能控制播放并显示封面，托盘基础功能正常，音量条外观可接受。导航/窗口改造检查点为 `7f4ae955`；用户随后报告 Linux 自绘窗口栏点击异常和闪烁，代码已移除依赖路由 Overlay 的 Tooltip，并在启动时恢复普通窗口状态、最小化/最大化/缩放能力。Linux GTK runner 现已改为单实例并在再次启动时激活现有窗口，但尚未编译或实测。按用户要求，最新修正的 release 和自动测试没有重跑，等用户自行启动验收。长队列/窗口尺寸、完整 MPRIS seek、关窗恢复/二次启动、干净安装与 Android 最终实机回归仍待验收。
...
| P4 | Linux 基础可用 | 用户确认 MPRIS 控制/封面与托盘基础功能；单实例激活代码已落但未验证；补完整 seek、托盘宿主失效、关窗恢复和退出边界 |
