...
| P2 | 桌面统一返回/前进栈和 Linux 自绘窗口栏已实现，等待用户复测 | 手机保留分支路由，桌面使用单 Navigator；无账户/服务器底栏与离线任务状态页；品牌区/主工具栏对齐、播放条贴边；新回归用例未运行，Ubuntu 交互待用户验收 |
| P3 | 首轮实现中 | 宽屏/紧凑工作区、队列右键、可关闭的封面动态背景已实现；5000 首定位和本轮用例未运行；长队列性能采样和真实窗口验收待补 |
| P4 | Linux 基础可用 | 用户确认 MPRIS 控制/封面与托盘基础功能；单实例、关窗选择、下载退出提示及 SetPosition/Seeked/error 隔离已编译；MPRIS 用例已加但未跑，托盘宿主失效与恢复仍待用户验收 |
| P5 | `c2496e94` 的 Linux bundle 与 `.deb` 已构建 | 尚未安装 `.deb` 或在干净 Ubuntu 播放；之后做 Android 回归；Windows CI 和 Windows 实机验收暂缓 |
