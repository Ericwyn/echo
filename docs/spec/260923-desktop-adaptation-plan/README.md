...
| P2 | 桌面统一返回/前进栈、滚动与搜索状态恢复和 Linux 自绘窗口栏已实现，等待用户复测 | 手机保留分支路由，桌面使用单 Navigator；无账户/服务器底栏与离线任务状态页；品牌区/主工具栏对齐、播放条贴边；前进复用 PageStorageBucket；其他页面筛选与新回归用例仍待验证 |
...
| P5 | `859745a6` 的 Linux bundle 与 `.deb` 已构建 | 尚未安装 `.deb` 或在干净 Ubuntu 播放；之后做 Android 回归；Windows CI 和 Windows 实机验收暂缓 |
