...
目前桌面侧栏、常驻播放条、歌词/队列工作区、Linux MPRIS 和托盘已有首轮实现。用户确认 Ubuntu 系统媒体卡片能控制播放并显示封面，托盘基础功能正常，音量条外观可接受。导航/窗口改造检查点为 `7f4ae955`；用户随后报告 Linux 自绘窗口栏点击异常和闪烁，代码已移除依赖路由 Overlay 的 Tooltip，并在启动时恢复普通窗口状态、最小化/最大化/缩放能力。Linux GTK runner 现已改为单实例并在再次启动时激活现有窗口，尚待用户实测。`4015f879` 的最新 Linux release bundle（约 59 MB）和 `.deb`（约 18 MB）已构建；只检查了包内文件与依赖元数据，没有安装或启动。最新桌面改动移除账户底栏重复弹窗，将线路和音乐库新增入口归到设置页；添加音乐库流程和设置历史仍待用户实测。按用户要求，不启动应用、不运行测试；窗口栏交互、单实例、状态恢复和 Android 最终实机回归留给用户验收。长队列、完整 MPRIS seek、托盘宿主异常和干净安装也仍待验收。
...
| P2 | 桌面统一返回/前进栈和 Linux 自绘窗口栏已实现，等待用户复测 | 手机保留分支路由，桌面使用单 Navigator；去掉重复搜索/返回/标题与账户弹窗，账户操作归到设置页；窗口视觉与操作等用户验收 |
| P3 | 首轮实现中 | 宽屏/紧凑工作区、桌面队列鼠标选择/播放/拖动，以及 Enter/Delete 与全局快捷键已实现；补键盘测试执行、长队列和真实窗口尺寸验收 |
| P4 | Linux 基础可用 | 用户确认 MPRIS 控制/封面与托盘基础功能；单实例与窗口尺寸/最大化恢复已在 `4015f879` 编译，交互待用户验证；补完整 seek、托盘宿主失效、关窗恢复和退出边界 |
| P5 | `4015f879` 的 Linux bundle 与 `.deb` 已构建 | 尚未安装 `.deb` 或在干净 Ubuntu 播放；之后做 Android 回归；Windows CI 和 Windows 实机验收暂缓 |
