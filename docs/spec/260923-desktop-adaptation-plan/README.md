...
最近应用代码检查点：`0e832ddb`（Linux 状态栏 watcher 退避重连及窗口退出失败恢复），前序 `c4bd0b6e` 补上共享队列起始索引保护。Linux 最新 `.deb` 为 `1.1.0+2053`/amd64，SHA-256 `f6e7b48e190b0cb3e696dbd57bc0a5ef875fc09f9357a2cc3d4a028e76b58450`；Android 本机签名 ARM64 APK versionCode `2054`，本机脚本由 `.git/info/exclude` 忽略。最新 Linux 证据见[生命周期恢复构建](evidence/260924-linux-lifecycle-recovery/README.md)。
...
| P4 | Linux 基础可用 | 用户确认 MPRIS 控制/封面与托盘基础功能；单实例、关窗选择、首次关闭到后台恢复提示、动态托盘控制、下载退出提示及 SetPosition/Seeked/error 隔离已编译；`0e832ddb` 增加 watcher 断线退避重连和 destroy 失败恢复，Linux release 已构建。新增用例未运行；关窗恢复、宿主重启和手动验收仍待用户完成 |
| P5 | Linux `1.1.0+2053` DEB 与 bundle ZIP、Android ARM64 APK 已构建 | Linux lifecycle recovery commit `0e832ddb` 在 `build/linux-lldtmp-2064` 构建；DEB SHA-256 `f6e7b48e190b0cb3e696dbd57bc0a5ef875fc09f9357a2cc3d4a028e76b58450`，standalone bundle ZIP SHA-256 `3d313cc02e489cfed707dd4980b10256511dca7e5529dc879265a8453965dcdb`，归档完整性通过。Android 本机签名 ARM64 APK versionCode `2054`，仅含 `arm64-v8a`，证书校验通过。产物未安装或启动；手动与性能验收仍由用户执行；Windows CI 暂缓。最新 Linux 证据：[生命周期恢复构建](evidence/260924-linux-lifecycle-recovery/README.md) |
