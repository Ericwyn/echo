...
| P2 | 桌面统一返回/前进栈及主要浏览状态恢复已实现，等待用户复测 | 手机保留分支路由，桌面使用单 Navigator；无账户/服务器底栏与离线任务状态页；品牌区/主工具栏对齐、播放条贴边；前进复用 PageStorageBucket，搜索输入、详情排序、收藏页签和歌手内容区可恢复；其他页面局部状态与新增回归仍待验证 |
...
| P5 | `52463122` 的 Linux bundle 与 `.deb` 已构建 | 尚未安装 `.deb` 或在干净 Ubuntu 播放；之后做 Android 回归；Windows CI 和 Windows 实机验收暂缓 |
