...
| 2026-09-24 / `46453bb2`、`b12aa608` | `PlaybackCommands` 新增 queue/preview/play-next/add-to-queue 意图；歌曲列表、搜索、下载、试听、歌曲动作、专辑/歌手/歌单和队列追加入口改由 `playbackCommandsProvider` 调用，收藏/切库/元数据刷新保持各自职责。未运行测试/analyze；Linux `1.1.0+2053` `.deb` 与 bundle ZIP、Android 签名 arm64-only APK versionCode `2053` release 编译通过 | 用户播放控制已统一经稳定 provider 委托给同一 `PlayerNotifier`；widget 回归、Android 后台行为和 Ubuntu 交互待用户验收 |
| 2026-09-24 / `c4bd0b6e` | `PlaybackCommands.playQueue` 在读取歌曲前将 `startIndex` 限制到有效范围；增加负索引与超长索引的恢复用例定义，按用户要求未运行。Linux `1.1.0+2053` `.deb`/bundle ZIP 与签名 Android arm64-only APK versionCode `2054` 编译通过 | 队列命令不再因越界 index 抛 `RangeError`；自动回归、长队列性能与真机验收待用户执行 |
| 2026-09-24 / `c7455cf0` | 将 `setLoopMode` 与 `setShuffleEnabled` 改为正交状态更新；增加 `playback_modes_v2` 持久化并由旧单值模式迁移；播放会话序列化两种状态；手机四模式循环保留，桌面栏分别切换随机与循环。新增 MPRIS/property、偏好迁移、会话恢复、shuffle+repeat-off 完结回归定义，未运行。Linux `1.1.0+2057` DEB/bundle 与本机签名 Android ARM64 APK versionCode `2055` 均构建、元数据/架构/证书校验通过 | Android/UI 与 GNOME 手动回归及自动测试仍待用户执行；Windows 未验证 |
| 2026-09-24 / `24e5319e` | `PlayerNotifier` 将随机/循环 setter、toggle、cycle 与恢复操作纳入串行队列；独立 setter 在前序更新完成后读取另一模式，避免并发 MPRIS property 写入丢状态。新增并发 setter 回归定义，未运行；Linux `1.1.0+2058` DEB/bundle 与签名 Android ARM64 APK versionCode `2056` 构建、元数据/架构/证书校验通过 | 自动回归和 Android/GNOME 手动行为待验；Windows 未验证 |
