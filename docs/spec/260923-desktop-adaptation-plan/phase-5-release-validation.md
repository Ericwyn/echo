...
状态：应用代码 `16add6b6` 已在全新 `build/linux-lldtmp-2036` CMake 目录中构建，使用系统 Clang 与从 Ubuntu 包解压到 `/tmp` 的真实 LLD 14；`c25e22aa` 支持从该 bundle 生成 Ubuntu `.deb`。当前 checkout 另已构建并签名 Android arm64-only release APK，签名与 package metadata 已校验；两端产物均未安装或启动。前置：P0–P4 仍有系统操作、恢复、性能和安装场景待验证。当前优先 Ubuntu/Linux；Windows CI 暂缓。
...
- 历史包 `build/linux/packages/echoes_1.1.0+26_amd64.deb` 与 `1.1.0+2027` 至 `+2035` 候选均已被替换。当前最新包为 `build/linux-lldtmp-2036/packages/echoes_1.1.0+2036_amd64.deb`，SHA-256 为 `2fd3587139e6b22544cf19e370406d119d5699466ad83884d4cfb4d1f478397d`；`dpkg-deb` 检查通过，仍未安装验证。干净系统播放依赖仍待 Ubuntu 实机记录。
