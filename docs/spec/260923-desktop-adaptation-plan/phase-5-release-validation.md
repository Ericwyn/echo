...
状态：应用代码 `94b7a2ab` 已在全新 `build/linux-lldtmp-2049` CMake 目录中构建，使用系统 Clang 与从 Ubuntu 包解压到 `/tmp` 的真实 LLD 14；`b8e3cd00` 加固 bundle 完整性检查并生成 Ubuntu `.deb`。当前 checkout 另已构建并签名 Android arm64-only release APK，versionCode `2038`，签名与 package metadata 已校验；两端产物均未安装或启动。前置：P0–P4 仍有系统操作、恢复、性能和安装场景待验证。当前优先 Ubuntu/Linux；Windows CI 暂缓。
...
- 历史包 `build/linux/packages/echoes_1.1.0+26_amd64.deb` 与 `1.1.0+2027` 至 `+2047` 候选均已被后续构建替换。当前最新包为 `build/linux-lldtmp-2049/packages/echoes_1.1.0+2048_amd64.deb`，SHA-256 为 `5e216ef9c8eea67eeb3de9de8d985d321d16bb29c74c5cde5c692a1199b2b048`；`dpkg-deb` 元数据检查通过，打包前的完整 bundle 校验也通过，仍未安装验证。干净系统播放依赖仍待 Ubuntu 实机记录。
