...
状态：应用代码 `82edffab` 已在全新 `build/linux-lldtmp-2037` CMake 目录中构建，使用系统 Clang 与从 Ubuntu 包解压到 `/tmp` 的真实 LLD 14；`c25e22aa` 支持从该 bundle 生成 Ubuntu `.deb`。当前 checkout 另已构建并签名 Android arm64-only release APK，签名与 package metadata 已校验；两端产物均未安装或启动。前置：P0–P4 仍有系统操作、恢复、性能和安装场景待验证。当前优先 Ubuntu/Linux；Windows CI 暂缓。
...
- 历史包 `build/linux/packages/echoes_1.1.0+26_amd64.deb` 与 `1.1.0+2027` 至 `+2036` 候选均已被替换。当前最新包为 `build/linux-lldtmp-2037/packages/echoes_1.1.0+2037_amd64.deb`，SHA-256 为 `2015a7e82345f197638e74ce298dd19c458df9d18f2f3c9fbc9c96c067d687e3`；`dpkg-deb` 检查通过，仍未安装验证。干净系统播放依赖仍待 Ubuntu 实机记录。
...
| Android ARM64 APK | `82edffab` / `pubspec.yaml` `1.1.0+2037` / `app-arm64-v8a-release.apk` | 本机专用 ARM64 脚本产物；APK version name/code 为 `1.1.0` / `4037`，仅含 `arm64-v8a`；本机 release 证书 SHA-256 `8604465c3ee1282b48a1b2759801f784ace32447d1188e0481fab0fe8ac74c94`；APK SHA-256 `f783417e4ac2fe32164d8cc10312122cbc9d6ac072ff41bc314e929887013bd2`。未安装、未启动、未运行 Flutter 测试；设备回归见 [Android release build evidence](evidence/260923-android-release-build/README.md) |
