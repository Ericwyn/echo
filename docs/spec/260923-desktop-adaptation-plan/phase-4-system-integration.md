...
状态：待实施。前置：P0 对应平台技术验证、P1 命令/快照契约；与 P2/P3 联调完成后才能认定桌面交互闭环。
...
P4-A 不依赖托盘存在；P4-B 的隐藏行为必须等托盘或其他恢复路径实际可用。Windows 没有真实桌面会话证据时只标注实现/编译状态，不标为已验收。
