...
| P4-A concurrent Shuffle/LoopStatus writes | 2026-09-24 / `24e5319e` | `PlayerNotifier` 串行执行共享播放模式写入，第二项只在前一项完成后读取当前状态；避免并发 D-Bus property setter 丢失另一项状态。新增并发回归定义，未运行；Linux `1.1.0+2058` 和签名 Android ARM64 versionCode `2056` 编译成功 | GNOME 同时或快速交错改变 Shuffle/LoopStatus 的行为仍待手测 |
