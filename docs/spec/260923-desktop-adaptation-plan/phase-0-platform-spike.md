...
| 2026-09-23 / `a9cb6989`，clean Linux + Android arm64 build | 收藏夹三个标签页滚动存储隔离后，在 `build/linux-lldtmp-2032` 全新 CMake build-dir 构建 `1.1.0+2032` amd64 `.deb`；本机脚本生成 version code `4032` 的 arm64-only 签名 APK | 构建成功；APK 证书/ABI/version 与 `.deb` 元数据、SHA-256 已核对。新增用例未运行，两端未安装/启动，用户负责实机验收；Windows 暂缓 |
| 2026-09-23 / `f2828730`，clean Linux + Android arm64 build | 曲库集合及全部歌曲两种排序模式的滚动存储隔离后，在 `build/linux-lldtmp-2033` 全新 CMake build-dir 构建 `1.1.0+2033` amd64 `.deb`；本机脚本生成 version code `4033` 的 arm64-only 签名 APK | 构建成功；APK 证书/ABI/version 与 `.deb` 元数据、SHA-256 已核对。新增用例未运行，两端未安装/启动，用户负责实机验收；Windows 暂缓 |
| 2026-09-23 / `871f95ed`，clean Linux + Android arm64 build | SongListPage 滚动签名缓存后，在 `build/linux-lldtmp-2034` 全新 CMake build-dir 构建 `1.1.0+2034` amd64 `.deb`；本机脚本生成 version code `4034` 的 arm64-only 签名 APK | 构建成功；APK 证书/ABI/version 与 `.deb` 元数据、SHA-256 已核对。性能回归未运行，两端未安装/启动，用户负责实机验收；Windows 暂缓 |
| 2026-09-24 / `6f0f80e6`，Linux + Android ARM64 release | 按音乐库隔离播放会话，切库时保存并停止旧播放器、清理系统媒体项；Android AudioService 使用进程级单例和 notifier 命令重绑。在 `build/linux-lldtmp-2052` 全新 CMake build-dir 构建 `1.1.0+2050` amd64 `.deb`；本机签名脚本构建 versionCode `2041` 的 arm64-only APK | 两端构建成功，包元数据、APK 版本/ABI/签名证书已核对；回归用例未运行，产物未安装或启动；切库行为和 Android 后台播放待用户实机验证；Windows 暂缓 |
| 2026-09-24 / `a6ee5e65`，Linux + Android ARM64 release | 添加音乐库登录成功后回到原发起页；编辑/删除音乐库按最近 Navigator 返回。全新 `build/linux-lldtmp-2053` 构建 `1.1.0+2051` amd64 `.deb`；本机签名脚本构建 versionCode `2042` 的 arm64-only APK | 两端构建成功，包元数据、APK 版本/ABI/签名证书已核对；导航回归用例未运行，产物未安装或启动；Ubuntu/Android 手动流程由用户验证；Windows 暂缓 |
| 2026-09-24 / `67f62407`，Linux + Android ARM64 release | 添加音乐库认证成功后先保存/停止旧库播放器，再激活新库；写库失败会恢复旧播放器并删除未激活记录。全新 `build/linux-lldtmp-2054` 构建 `1.1.0+2052` amd64 `.deb`；本机签名脚本构建 versionCode `2043` 的 arm64-only APK | 两端构建成功，包元数据、APK 版本/ABI/签名证书已核对；回归用例未运行，产物未安装或启动；Ubuntu/Android 手动流程由用户验证；Windows 暂缓 |
| 2026-09-24 / `d85868f1`，Linux + Android ARM64 release | 删除非活动库时维持当前活动库；删除活动库且替代激活失败时登出，不留失效 library 引用。全新 `build/linux-lldtmp-2055` 构建 `1.1.0+2053` amd64 `.deb`；本机签名脚本构建 versionCode `2044` 的 arm64-only APK | 两端构建成功，包元数据、APK 版本/ABI/签名证书已核对；未安装/启动、未运行测试；删除和失败恢复由用户手动验证；Windows 暂缓 |
