...
| 2026-09-23 / `c25e22aa`，clean Linux build | 使用隔离 Flutter 配置与全新 `build/linux-noshim` 目录执行 release build；CMake 使用系统 clang/GNU ld；为打包脚本增加 `ECHO_LINUX_BUNDLE_DIR`，从隔离 bundle 生成 `.deb`，核对 ELF、包依赖、入口文件、图标和 SHA-256 | 全新配置编译和打包成功，扫描 build tree 无 `/tmp` shim 引用；未安装、未启动、未运行 Flutter tests/analyze。MPRIS seek、托盘恢复、单实例、干净安装仍待实机验证；Windows 暂缓 |
