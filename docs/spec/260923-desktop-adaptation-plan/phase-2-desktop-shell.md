...
状态：首轮实现中。按用户指定分组的宽屏侧栏、统一字号、与播放条等高的静态账户身份区、持久桌面播放条和保留浏览分支的播放器工作区已落代码。桌面账户区不再打开重复的设置弹窗；线路选择与添加音乐库集中放在侧栏可达的设置页，音乐库切换/编辑也在那里提供。桌面现使用独立 Navigator：侧栏主目的地替换当前主页面，内容详情共享同一返回/前进栈，根页面为音乐流；Android 继续使用 `StatefulShellRoute`。Linux 隐藏 GNOME GTK 标题栏和原生窗口框，由应用根部绘制覆盖登录与主界面的窗口栏及边缘缩放区；主应用页面在内容 shell 内显示返回/前进/搜索工具栏。窗口最小逻辑尺寸 840×560，避免落入 Android 风格布局。自动用例覆盖主目的地连续切换、详情逐层返回、前进恢复、重复返回箭头消除和窗口控制语义。用户实测反馈上一版窗口栏按钮不可点/闪烁、窗口缩放异常；已移除放在 Navigator Overlay 外的 Tooltip，启动时清除置顶并显式恢复缩放、最大化和最小化能力。最新 UI 修正隐藏音乐流页面重复的搜索入口、搜索页重复返回箭头、歌单页重复标题，并把桌面四处 chrome 分隔线改为弱化的 divider token；修正已包含于 `4015f879` Linux bundle，待用户运行验收。前进通过保存的路由 builder 重建页面；Ubuntu 实际拖动/缩放、滚动状态、不同尺寸/DPI尚待验证。Windows 暂保留原生标题栏，之后单独验证。
...
5. 宽屏解除桌面 `Scaffold.drawer`，侧栏底部仅展示账户身份，不提供重复弹窗。下载/离线/设置等管理页由侧栏直达；线路切换、添加/切换/编辑音乐库集中到设置页。需要认证的添加音乐库流程仍走应用认证路由。手机继续使用原主导航与抽屉 presenter。
...
- A01：宽窗、图标栏与窗口连续缩放只出现一套主导航；侧栏账户区为静态身份信息，设置/下载均可直达。
- A18：桌面账户区不再弹出重复功能菜单；线路选择与音乐库新增/切换/编辑可从设置页到达，添加流程退出后回到设置/桌面历史。
