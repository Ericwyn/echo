...
| 2026-09-23 / 待提交 | Linux 在首帧前隐藏 GNOME GTK 标题栏；返回/前进/搜索与标题拖动区、最小化/最大化/还原/关闭合并进全宽自绘顶栏；expanded 页面移除重复返回箭头；桌面窗口最小宽度提高到 840 逻辑像素。补回归测试并完成 Linux release/X11 手动验证后记录提交 | Android 窗口约束不变；Linux X11 拖动、尺寸下限和窗口按钮待实机确认；Wayland及 Windows 仍未测 |
