...
| 搜索、筛选、滚动 | 页面/分支 | 搜索草稿/已提交查询、音乐流随心听展开状态、曲库歌单排序、专辑/歌单详情排序、收藏页签及歌曲/专辑/歌手各自滚动位置、歌手内容区和滚动位置在桌面前进重建后恢复；其余页面局部状态继续逐页接入 |
