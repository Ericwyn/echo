...
- R11：Linux 的 Echo 标识、返回/前进、搜索和窗口按钮在同一根级顶栏；侧栏与主面板不再绘制第二行；底部桌面播放条与主面板左右/下边缘无额外间距。
...
| 2026-09-24 / `2e15f875` | Linux 根级顶栏合并品牌、返回/前进、搜索和窗口按钮；移除壳内双顶栏。去掉会裁出黑角的 `VirtualWindowFrame`，保留 GTK CSD 与原生缩放边缘，并用 GTK CSS 绘制直角和阴影。Linux `1.1.0+2059` release bundle、DEB、ZIP 已构建；格式化与 diff 检查通过，未启动或运行测试 | Ubuntu 实际阴影、直角、拖动/缩放和顶栏点击仍由用户验收；[构建证据](evidence/260924-single-linux-titlebar/README.md)；Windows/Android 不走 Linux CSD 路径 |
