...
状态：首轮实现中。宽屏统一侧栏、子页快捷入口、账户弹窗、持久桌面播放条和保留浏览分支的播放器工作区已落代码；Ubuntu release/实机、路线历史前进与完整 DPI 验收尚待验证。前置：P1 共享接口已建立；Android 全量回归尚待 P5。后续：P3 在稳定 shell 内切换歌词/队列；P4 接入系统命令。
...
| 日期 / 提交 | 实际修改与检查 | 结果 / 遗留 |
| --- | --- | --- |
| 2026-09-23 / 首轮桌面 UI | 宽屏关闭 `Scaffold.drawer`，侧栏整合主目的地、搜索/歌曲/歌手/专辑/收藏/管理入口及账户菜单；浏览分支以 Offstage 保留；增加桌面播放条、固定封面工作区和 shell 内直接子页入口。运行侧栏/主导航/队列/全屏播放器/MiniPlayer 五组 widget 测试，共 51 项通过 | 未验证 Ubuntu release 与 DPI 实机；尚无全局前进历史按钮，路线历史/账户弹窗需实机验证；阶段仍进行中 |
