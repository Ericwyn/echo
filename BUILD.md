...
`pubspec.yaml` 中的 `version: 1.1.0+2065` 是当前示例：`1.1.0` 是 Android `versionName`，`2065` 是基础 `versionCode`，Linux 打包脚本使用完整的 `1.1.0+2065` 作为 DEB 版本。发布新版本前递增构建号；安装更新时，新 APK 的**实际** `versionCode` 必须大于设备上已有的包。
...
Flutter 的 `--split-per-abi` 会为不同 ABI 的 APK 添加版本码偏移；当前 Flutter 的 ARM64 拆分包在基础构建号上加 **2000**。例如基础构建号 `2065` 对应的 ARM64 拆分 APK 实际版本码是 `4065`；不拆分的通用 APK 则使用基础构建号。**从拆分包切换到通用包**时，通用包需要选用高于已安装拆分包的构建号，否则 Android 会拒绝降级。可通过 `aapt dump badging <APK 路径>` 检查实际 `versionCode`，必要时在构建命令中使用 `--build-number=<更大的整数>`；之后仍应将 `pubspec.yaml` 的构建号同步提高，避免下一次构建倒退。
...
直接运行 bundle 中的 `echoes` 时，程序会从 bundle 资源设置 GTK 窗口图标；完整应用菜单项和主题图标仍由 DEB 安装的 `.desktop` 文件提供。当前 Ubuntu 托盘中键可触发“显示 Echo”；双击激活还取决于托盘宿主与系统 Ayatana 库是否提供 `Activate`，Ubuntu 22.04 的 0.5.90 库不提供该方法，菜单中的“显示 Echo”仍可使用。
