...
// Available in newer libayatana-appindicator releases. Older Ubuntu versions
// use the secondary activation menu target configured below instead.
void _on_indicator_activate(AppIndicator* indicator, gint x, gint y,
                            gpointer user_data) {
  auto* plugin = TRAY_MANAGER_PLUGIN(user_data);
  fl_method_channel_invoke_method(plugin->channel, "onTrayIconMouseDown",
...
static FlMethodResponse* set_icon(TrayManagerPlugin* self, FlValue* args) {
  ...
  if (!indicator) {
    ...
    // 0.5.90 has no primary activation signal, but newer libraries can
    // forward a tray-host activation (including GNOME double-click) to Dart.
    if (g_signal_lookup("activate", G_OBJECT_TYPE(indicator)) != 0) {
      g_signal_connect(indicator, "activate",
                       G_CALLBACK(_on_indicator_activate), self);
    }
...
static FlMethodResponse* set_context_menu(TrayManagerPlugin* self,
  ...
  // Echoes puts "Show Echo" first. On Linux tray hosts, a middle-click
  // dispatches SecondaryActivate to this item instead of opening the menu.
  GList* items = gtk_container_get_children(GTK_CONTAINER(menu));
  if (items != nullptr) {
    app_indicator_set_secondary_activate_target(indicator,
