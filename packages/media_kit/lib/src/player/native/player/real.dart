  ...
  Future<void> _create() {
    return lock.synchronized(() async {
      ...
      final properties = <String, String>{
        ...
        'dither': 'no',
        'cache': 'yes',
        // Ubuntu 22.04 ships libmpv 0.34, which requires an explicit cache-dir
        // for disk caching. This app does not configure a shared MPV cache
        // directory, so keep Linux network buffering in the bounded memory
        // cache instead of making lavf fail to create its temporary cache.
        'cache-on-disk': Platform.isLinux ? 'no' : 'yes',
        'hr-seek': 'yes',
        ...
        // OpenSL ES audio output seems to be broken on some of these.
          ...
          'ao': 'null',
        // `subs-fallback` is not available in Ubuntu 22.04's libmpv 0.34.
        // Linux playback here is audio-only, so the subtitle option is unused.
        if (!Platform.isLinux) 'subs-fallback': 'yes',
