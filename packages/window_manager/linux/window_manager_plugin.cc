...
void emit_button_release(WindowManagerPlugin* self) {
  // GTK expects a button event to belong to a realized window and input
  // device. The synthetic release after a window drag otherwise reaches GTK
  // with a null device and triggers gdk_device_get_axis assertions.
  GdkWindow* window = gtk_widget_get_window(self->_event_box);
  if (window == nullptr) {
    return;
  }
  ...
  newEvent->window = GDK_WINDOW(g_object_ref(window));
  newEvent->send_event = TRUE;
  gdk_event_set_device((GdkEvent*)newEvent, pointer);
  newEvent->x = self->_event_button.x;
  newEvent->y = self->_event_button.y;
  newEvent->x_root = self->_event_button.x_root;
  newEvent->y_root = self->_event_button.y_root;
